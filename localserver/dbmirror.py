...
def get_arguments():
	...
	"""
     ...
     parser = argparse.ArgumentParser(description = ("copy items from different inboxes [-options]"))
...
def get_max_index_from_table(table):
	"""
	**Description:**
	...
	"""
	command ="mysql -u %s -h %s -e 'select max(inbox_id) from %s.smsinbox_%s where gsm_id!=1' -p%s" % (c.db["user"],c.dbhost["local"],c.db["name"],table,c.db["password"])
	p = subprocess.Popen(command, stdout=subprocess.PIPE, shell=True, stderr=subprocess.STDOUT)
	out, err = p.communicate()
...
def get_last_copied_index():
	...
	"""
	f_index = open("/home/dewsl/scriptlogs/user_inbox_index.tmp")
...
def import_sql_file_to_dyna(table,max_inbox_id,max_index_last_copied):
	"""
        **Description:**
    	...
    	copy_query = ("SELECT t1.ts_received as 'timestamp', t2.sim_num, t1.sms_msg, 'UNREAD' "
    	...
    	# Rewrite the data inside the user_inbox_index.tmp
    	f_index = open("/home/dewsl/scriptlogs/user_inbox_index.tmp","wb")
...
def sandbox_to_dyna():
    ...
    if max_inbox_id > max_index_last_copied:
		import_sql_file_to_dyna(table,max_index_last_copied,max_inbox_id)
...
def main():
    """
        **Description:**
        ...
        .. note:: To run in terminal **python dbmirror.py -m1** for dyna to sandbox and **python dbmirror.py -m2** for sanbox to dyna.
    """
    args = get_arguments()
