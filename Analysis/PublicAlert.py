...
def SitePublicAlert(PublicAlert, window):
    ...
    PublicAlert.loc[alert_index] = [window.end, PublicAlert.site.values[0], 'public', public_alert]
    
    SitePublicAlert = PublicAlert.loc[PublicAlert.site == site]
    
    alert_toDB(SitePublicAlert, 'site_level_alert')
...
def main():
    ...
    PublicAlert = pd.DataFrame({'timestamp': [window.end]*len(q.GetRainProps()), 'site': q.GetRainProps().name.values, 'source': ['public']*len(q.GetRainProps()), 'alert': [np.nan]*len(q.GetRainProps())})
    PublicAlert = PublicAlert[['timestamp', 'site', 'source', 'alert']]
