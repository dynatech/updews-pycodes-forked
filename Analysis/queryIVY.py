...
def GetRawAccelData(siteid = "", fromTime = "", toTime = "", targetnode =""):
    ...
    if (len(siteid) == 5):
        ...
        query ="""Select ts,'%s' as 'name',node_id,xval,yval,zval,batt,if (type_num in (11,32),1,2) as 'accel' from 
                (SELECT ts,'%s' as 'name',node_id,xval,yval,zval,batt,type_num from tilt_%s""" %(siteid,siteid,siteid)
        ...
        targetnode_query=" AND node_id=%d" %targetnode
        query= query + targetnode_query + ") raw"
        
        query= query + " where type_num in (11,32) or type_num in (12,33) order by accel, ts"


    elif (len(siteid) == 4):
        
        query ="""Select ts,'%s' as 'name',node_id,xval,yval,zval, 1 as 'accel' from tilt_%s""" %(siteid,siteid)
    ...
    if (len(siteid) == 5):
        ...
        df.columns = ['ts','name','id','x','y','z','batt','accel']
        df.ts = pd.to_datetime(df.ts)
        return df
