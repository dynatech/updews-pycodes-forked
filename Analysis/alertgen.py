...
def alert_toDB(df, table_name, window):
    ...
    query = "SELECT timestamp, site, source, alert FROM senslopedb.%s WHERE site = '%s' and timestamp <= '%s' AND updateTS >= '%s' ORDER BY timestamp DESC LIMIT 1" %(table_name, df.site.values[0], window.end, window.end)
