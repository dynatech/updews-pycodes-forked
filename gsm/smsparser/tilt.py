...
def process_v1(sms_raw):
    sms = pre_process_v1(sms_raw)
    line = text
    txtdatetime = sms.ts
    sender = sms.simnum
...
def process_v2(sms,text):
    ...
    for pattern in ts_patterns:
        try:
            timestamp = dt.strptime(ts,pattern).strftime('%Y-%m-%d %H:%M:%s')
            break
        except ValueError:
