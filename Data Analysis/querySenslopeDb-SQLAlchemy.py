# -*- coding: utf-8 -*-
"""
Created on Wed Mar 16 09:14:51 2016
...
def SenslopeDBConnect(nameDB):
    while True:
        try:
            url = URL('mysql', Userdb, Passdb, Hostdb, None, nameDB)
            db_url = make_url(url)
            #db_url = "mysql://root:senslope@localhost/" + nameDB
...
def DoesTableExist(table_name):
    ...
    row = cur.execute("SHOW TABLES LIKE %s", (table_name,))
    ...
    if row.rowcount > 0:
        ...
        db.close()
        #print "Table {0} does NOT exists.format(table_name)
        return False
...
def GetLatestTimestamp2(table_name):
    ...
    try:
        row = cur.execute("SELECT MAX(timestamp) FROM %s" % (table_name))
    except:
        print "Error in getting maximum timestamp.", sys.exc_info()
...
def PushDBDataFrame(df,table_name):     
    ...
    try:
        ...
        session.flush()        
        print "Data succesfully pushed."
    except:
        print "Error occured in pushing data on database.", sys.exc_info()
        session.rollback()
    db.close()
...
SenslopeDBConnect('senslopedb')
DoesTableExist('accel')
GetLatestTimestamp2("accel")
query = "SELECT public_alert_desc FROM lut_alerts"
a = GetDBResultset(query)
b = GetDBDataFrame(query)
testPush = list([('', "Foo", "Bar", "foobar", "foo123", "foo@bar.com")])
testDfPush = pd.DataFrame(data=testPush, columns=['id', 'first_name', 'last_name', 'username', 'password', 'email_address'])
#PushDBDataFrame(testDfPush, 'membership')
c = GetCoordsList()
d = GetSensorList()
e = GetSensorDF()
f = GetRainList()
