...
for s in range(len(sensors)):
    ...
    df = qs.GetRawAccelData(siteid = targetTable, fromTime = "2015-10-15", msgid = 33, targetnode = node)
    ...
    if numElements > 0:
        ...
        if numFiltered > 0:
            print "Number of %s filtered elements: %s" % (targetTable, numFiltered)
            #print df_filtered
        else:
            print "No valid filtered data"
