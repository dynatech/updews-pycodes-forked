...
def main(time_now = datetime.now(), mysql=True, to_csv=False):
    ...
    if len(sent_sched) != 0:        
        unqueued_unsent = sent_sched.loc[(sent_sched[col_name].isnull()) | (~sent_sched.unwritten_info.isnull()) | (~sent_sched.tagged.isnull()), ['rain_site_code', 'fullname', 'unwritten_info', 'tagged']].drop_duplicates()
        unqueued_unsent = unqueued_unsent.fillna('')
        msg = 'un{name} Rain Info ({ts})'.format(name=name, ts=start)
        if len(unqueued_unsent) != 0:
            msg += ':\n'
            msg += '\n'.join(sorted(unqueued_unsent.apply(lambda row: (row.rain_site_code).upper() + ' ' + row.fullname + ' ' + row.unwritten_info + ' ' + row.tagged, axis=1).values))
        else:
            msg = 'No ' + msg
        msg = (msg[0].upper() + msg[1:]).replace('  ', ' ')
    else:
